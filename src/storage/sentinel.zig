...
pub const SentinelState = struct {
    ...
    /// Returns a tuple: (can_reach: bool, known_sentinels: usize, quorum: u8)
    /// can_reach is true if: (this Sentinel + known Sentinels) >= quorum
    pub fn checkQuorum(
        self: *SentinelState,
        master_name: []const u8,
    ) !struct { can_reach: bool, known_sentinels: usize, quorum: u8 } {
        const master = self.monitored_masters.getPtr(master_name) orelse return error.MasterNotFound;

        // Count: this Sentinel (1) + other known Sentinels
        const known_sentinels = master.sentinels.count();
        const total_sentinels = 1 + known_sentinels; // Include this Sentinel

        // Can reach quorum if total >= quorum
        const can_reach = total_sentinels >= master.quorum;

        return .{
            .can_reach = can_reach,
            .known_sentinels = known_sentinels,
            .quorum = master.quorum,
        };
    }

    /// Flush Sentinel configuration to disk
    /// Writes monitored masters and their sentinels to sentinel.conf file
    /// Returns error if write fails or path is invalid
    pub fn flushConfig(self: *SentinelState, config_path: []const u8) !void {
        ...
        // Write Sentinel ID
        try writer.print("# Sentinel ID: {s}\n# Generated by Zoltraak Sentinel\n\n", .{self.myid});
        ...
        while (it.next()) |entry| {
            ...
            try writer.print("sentinel monitor {s} {s} {d} {d}\n", .{
                master.name,
                master.ip,
                master.port,
                master.quorum,
            ...
            // SENTINEL down-after-milliseconds <name> <value>
            try writer.print("sentinel down-after-milliseconds {s} {d}\n", .{
                master.name,
                master.down_after_milliseconds,
            });

            // Write registered Sentinels for this master (for documentation only)
            if (master.sentinels.count() > 0) {
                try writer.print("# Known Sentinels for {s}:\n", .{master.name});

                var sentinel_it = master.sentinels.valueIterator();
                while (sentinel_it.next()) |sentinel_info| {
        ...
        // Write buffer to file and sync
        try file.writeAll(buf.items);
        try file.sync();
    }

    /// Set a configuration parameter for a master
    /// Supported parameters: quorum, down-after-milliseconds
    /// Returns error if master doesn't exist or parameter is invalid
    pub fn setMasterOption(
        self: *SentinelState,
        master_name: []const u8,
        option: []const u8,
        value: []const u8,
        ...
        if (std.mem.eql(u8, option, "quorum")) {
            // Parse quorum value
            const quorum = std.fmt.parseInt(u8, value, 10) catch return error.InvalidValue;
            if (quorum == 0) return error.InvalidValue; // Quorum must be > 0
            master.quorum = quorum;
        } else if (std.mem.eql(u8, option, "down-after-milliseconds")) {
            // Parse down-after-milliseconds value
            const millis = std.fmt.parseInt(u64, value, 10) catch return error.InvalidValue;
            if (millis == 0) return error.InvalidValue; // Must be > 0
            master.down_after_milliseconds = millis;
        } else {
            // Unsupported option
...
test "SentinelState.checkQuorum: can reach quorum with sufficient sentinels" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Add 1 other sentinel (total: this + 1 = 2)
    try sentinel.registerSentinel("mymaster", "s1", "127.0.0.2", 26379);

    // Check quorum
    const result = try sentinel.checkQuorum("mymaster");

    // Should be able to reach quorum (2 >= 2)
    try std.testing.expectEqual(true, result.can_reach);
    try std.testing.expectEqual(@as(usize, 1), result.known_sentinels);
    try std.testing.expectEqual(@as(u8, 2), result.quorum);
}

test "SentinelState.checkQuorum: cannot reach quorum with insufficient sentinels" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();

    // Add master with quorum of 3
    try sentinel.monitorMaster("mymaster", "127.0.0.1", 6379, 3);
    ...
    // Should NOT be able to reach quorum (2 < 3)
    try std.testing.expectEqual(false, result.can_reach);
    try std.testing.expectEqual(@as(usize, 1), result.known_sentinels);
    try std.testing.expectEqual(@as(u8, 3), result.quorum);
}

test "SentinelState.checkQuorum: includes this sentinel in count" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();

    // Add master with quorum of 1
    try sentinel.monitorMaster("mymaster", "127.0.0.1", 6379, 1);
    ...
    // Should be able to reach quorum (this sentinel counts)
    try std.testing.expectEqual(true, result.can_reach);
    try std.testing.expectEqual(@as(usize, 0), result.known_sentinels);
    try std.testing.expectEqual(@as(u8, 1), result.quorum);
}

test "SentinelState.checkQuorum: returns error for unknown master" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();

    const result = sentinel.checkQuorum("nonexistent");
    try std.testing.expectError(error.MasterNotFound, result);
}

// ============================================================================
// SENTINEL FLUSHCONFIG Tests
// ============================================================================

test "SentinelState.flushConfig: writes config to file" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Flush config
    const config_path = "test_sentinel.conf";
    try sentinel.flushConfig(config_path);
    ...
    // Verify content contains monitor directive
    try std.testing.expect(std.mem.indexOf(u8, content, "sentinel monitor mymaster") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "127.0.0.1 6379 2") != null);
}

test "SentinelState.flushConfig: rejects path with directory traversal" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();

    // Attempt to flush with .. in path
    const result = sentinel.flushConfig("../../../etc/passwd");
    try std.testing.expectError(error.InvalidPath, result);
}

test "SentinelState.flushConfig: writes multiple masters" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Flush config
    const config_path = "test_sentinel_multi.conf";
    try sentinel.flushConfig(config_path);
    defer std.fs.cwd().deleteFile(config_path) catch {};
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "sentinel monitor master1") != null);
    try std.testing.expect(std.mem.indexOf(u8, content, "sentinel monitor master2") != null);
}
...
test "SentinelState.setMasterOption: sets quorum" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Set quorum to 5
    try sentinel.setMasterOption("mymaster", "quorum", "5");

    const master = sentinel.getMaster("mymaster").?;
    try std.testing.expectEqual(@as(u8, 5), master.quorum);
}

test "SentinelState.setMasterOption: sets down-after-milliseconds" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Set down-after-milliseconds to 60000
    try sentinel.setMasterOption("mymaster", "down-after-milliseconds", "60000");

    const master = sentinel.getMaster("mymaster").?;
    try std.testing.expectEqual(@as(u64, 60000), master.down_after_milliseconds);
}

test "SentinelState.setMasterOption: rejects zero quorum" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Attempt to set quorum to 0
    const result = sentinel.setMasterOption("mymaster", "quorum", "0");
    try std.testing.expectError(error.InvalidValue, result);

    // Quorum should remain unchanged
    const master = sentinel.getMaster("mymaster").?;
    try std.testing.expectEqual(@as(u8, 2), master.quorum);
}

test "SentinelState.setMasterOption: rejects zero down-after-milliseconds" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Attempt to set down-after-milliseconds to 0
    const result = sentinel.setMasterOption("mymaster", "down-after-milliseconds", "0");
    try std.testing.expectError(error.InvalidValue, result);

    // Should remain unchanged (default 30000)
    const master = sentinel.getMaster("mymaster").?;
    try std.testing.expectEqual(@as(u64, 30000), master.down_after_milliseconds);
}

test "SentinelState.setMasterOption: rejects invalid value format" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Attempt to set quorum to invalid string
    const result = sentinel.setMasterOption("mymaster", "quorum", "abc");
    try std.testing.expectError(error.InvalidValue, result);
}

test "SentinelState.setMasterOption: rejects unsupported option" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();
    ...
    // Attempt to set unsupported option
    const result = sentinel.setMasterOption("mymaster", "unknown-option", "123");
    try std.testing.expectError(error.UnsupportedOption, result);
}

test "SentinelState.setMasterOption: returns error for unknown master" {
    const allocator = std.testing.allocator;
    var sentinel = SentinelState.init(allocator);
    defer sentinel.deinit();

    const result = sentinel.setMasterOption("nonexistent", "quorum", "3");
    try std.testing.expectError(error.MasterNotFound, result);
}
